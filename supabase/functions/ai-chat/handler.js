...
Keep responses concise and actionable. Use bullet points. Always tie advice to actual athlete data.\n\nAnswer in short plain sentences. Prefer simple correct answers over fancy prose. No filler.`}function C(n,l){const g=n.full_name??"Athlete",c=n.belt??"white",o=n.technique_summary,m=n.protocol,s=n.red_techniques,h=n.worst_joints,a=n.green_techniques,r=n.yellow_techniques,p=n.saved_game_plans,u=(a??[]).reduce((e,t)=>(e[t.category]||(e[t.category]=[]),e[t.category].push(t.name),e),{}),f=Object.entries(u).length>0?Object.entries(u).map(([e,t])=>`  ${e}: ${t.join(", ")}`).join(`
`):"  Technique readiness pending — sport pack techniques not loaded for this account yet. Assessment is on file.",d=r&&r.length>0?r.map(e=>{const t=(e.limiting_joints??[]).map(b=>R(b)).join(", ");return`  ${e.name} (${e.category})${t?` - ${t} limiting`:""}`}).join(`
`):"  Technique readiness pending — sport pack techniques not loaded for this account yet. Assessment is on file.",w=s&&s.length>0?s.slice(0,8).map(e=>{const t=e,b=(t.limiting_joints??[]).map(E=>R(E)).join(", ");return`  ${t.name} (${t.belt} belt) - ${b} limiting`}).join(`
...
${m?.map(e=>{const t=e;return`  ${t.joint??t.jointKey}: ${t.exercise} - ${t.sets}x${t.reps} (${t.cue??t.coaching_cue})`}).join(`
`)??"  Daily plan pending generation for this assessment (or no assessment yet)."}
